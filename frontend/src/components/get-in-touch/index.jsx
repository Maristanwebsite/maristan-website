const GetInTouch = ({ about, data }) => {
	return (
		<section className={`px-4 ${about && 'pb-[75px]'} mb-[35px]`}>
			<div
				className="max-w-[1440px] mx-auto  rounded-[32px] bg-cover bg-no-repeat md:bg-left "
				style={{
					backgroundImage: 'url("/images/getintouch-bg.png" )',
				}}
			>
				<div className=" flex flex-col lg:flex-row justify-between items-center px-[40px] pl-[50px] py-[40px] lg:py-[75px] lg:px-[50px] xl:px-[75px]  lg:gap-x-[40px] xl:gap-x-[75px] ">
					<div className="lg:w-[54%] mb-[24px] lg:mb-0 sm:text-center lg:text-start">
					...
					<div className=" h-full justify-center  sm:mx-auto lg:mx-0 flex flex-col sm:text-center lg:text-start">
						<div className="w-full flex flex-col sm:flex-row gap-x-[12px] mb-[12px]">
							<a
								target="_blank"
								href={`${data?.appotment?.link || data?.cta[0]?.link}`}
							...
							{/* <a href={`tel:+${data?.call?.link || data?.cta[1]?.link}`}>
