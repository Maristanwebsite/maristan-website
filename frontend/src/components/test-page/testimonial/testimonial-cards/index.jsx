...
const socialLinks = [
  { icon: <Instagram fill="#BCA692" />, Link: "https://www.instagram.com/maristan_org" },
  { icon: <Facebook fill="#BCA692" />, Link: "https://www.facebook.com/maristanorg" },
  { icon: <Twitter fill="#BCA692" />, Link: "https://twitter.com/maristan_org" },
  { icon: <Youtube fill="#BCA692" />, Link: "https://www.youtube.com/channel/UCitimhmbu395HNhgW7YFWbg" },
]
  ...
  return (
    <section className="flex lg:flex-row flex-col lg:justify-between lg:px-[38px] text-center lg:text-start items-center">
      <div className="max-w-[530px]">
        ...
        <p className="text-[14px] leading-[16px] text-[#8F1A1D] ff-convergence uppercase mt-[40px]">
          {data?.tagline}
        </p>
        ...
        <div className="flex items-center gap-x-[7px] mt-[40px]">
          {socialLinks?.map((item, index) => (
              <a
                  ...
                  href={`${item?.Link}`} target="_blank">{item.icon}
              </a>
          ))}
        </div>
      </div>
