			...
			<div className="relative max-w-[1440px] mx-auto">
				<div className="max-w-[651px] mx-auto ">
					...
					<div className="mt-[31px] sm:flex gap-x-[12px] justify-center ff-overpass">
						<a href={`${data?.listLinks[0]?.link}`} target="_blank">
							<p className="inline-flex gap-[19px] items-center py-[16px] px-[24px] w-full sm:w-auto bg-[#8F1A1D] text-[#FFFFFF] font-[600]  leading-[20px] uppercase rounded-[8px] hover:bg-[#a1261b] duration-200 transition-all">
								{data?.listLinks[0]?.name}
							</p>
						</a>

						<a target="_blank" href={data?.cta?.[1]?.link}>
							<button className="group flex items-center justify-center mt-[12px] sm:mt-0 w-full sm:w-auto hover:bg-[#e5dad1] transition-all duration-300 gap-x-[18px] py-[16px] border-[1px] border-[#ffff] px-[24px] text-[#ffff] hover:text-[#000000]  font-[600] leading-[20px] uppercase rounded-[8px]">
								<Dial className="fill-[white] group-hover:fill-black " />
								{data?.listLinks[1]?.name}
							</button>
						</a>
					</div>

					<p className="ff-overpass text-[#ffff] opacity-[0.8] mt-[40px] sm:max-w-[414px] mx-auto sm:text-center pl-[40px] sm:pl-0">
